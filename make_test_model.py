#!/usr/bin/env python3
"""make_test_model.py — Generate a synthetic mini-model in safetensors format.

Creates a directory with config.json + model.safetensors containing random
weights with the correct dimensions to test Picchio's loader end-to-end.

The model is SMALL (D=64, 2 layers, 4 experts) — a few MB.
It does not generate sensible text, but validates that:
  1. config.json is read correctly
  2. The safetensors are opened and parsed
  3. The weights are loaded into the right structs
  4. The forward pass runs without crashing
...
try:
    ...
    print("Install safetensors: pip install safetensors numpy")
    sys.exit(1)


def make_test_model(output_dir: str = "test_model"):
    """Generate a GPT-OSS-like mini-model for testing."""
    ...
    V = 32           # vocab_size (small for testing)
    ...
    print(f"  or: MODEL={output_dir} ./picchio.exe")
